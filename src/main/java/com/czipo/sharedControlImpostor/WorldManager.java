...
public class WorldManager {
    ...
    public boolean loadExistingSurvivalWorld() {
        ...
        if (folders != null) {
            for (java.io.File f : folders) {
                // Only consider the base survival world folder for finding the newest session
                if (f.isDirectory() && f.getName().startsWith("survival_game_") 
                        && !f.getName().endsWith("_nether") && !f.getName().endsWith("_the_end")) {
        ...
        if (newest != null) {
            ...
            if (survivalWorld != null) {
                ...
                // Also load associated nether and end worlds if they exist
                String netherName = survivalWorldName + "_nether";
                if (new java.io.File(worldContainer, netherName).exists()) {
                    survivalNetherWorld = new WorldCreator(netherName)
                            .environment(World.Environment.NETHER)
                            .createWorld();
                    if (survivalNetherWorld != null) {
                        survivalNetherWorld.setGameRule(GameRule.KEEP_INVENTORY, true);
                    }
                }

                String endName = survivalWorldName + "_the_end";
                if (new java.io.File(worldContainer, endName).exists()) {
                    survivalEndWorld = new WorldCreator(endName)
                            .environment(World.Environment.THE_END)
                            .createWorld();
                    if (survivalEndWorld != null) {
                        survivalEndWorld.setGameRule(GameRule.KEEP_INVENTORY, true);
                    }
                }
    ...
    public void createSurvivalWorld() {
        ...
        if (survivalWorld != null) {
            ...
            // Create the linked nether world
            String netherName = survivalWorldName + "_nether";
            survivalNetherWorld = new WorldCreator(netherName)
                    .environment(World.Environment.NETHER)
                    .seed(survivalWorld.getSeed())
                    .generateStructures(true)
                    .createWorld();
            if (survivalNetherWorld != null) {
                survivalNetherWorld.setGameRule(GameRule.KEEP_INVENTORY, true);
                survivalNetherWorld.setDifficulty(org.bukkit.Difficulty.NORMAL);
                Bukkit.getScheduler().runTask(plugin, () -> {
                    survivalNetherWorld.setGameRule(GameRule.SEND_COMMAND_FEEDBACK, false);
                    survivalNetherWorld.setGameRule(GameRule.ANNOUNCE_ADVANCEMENTS, false);
                    survivalNetherWorld.setGameRule(GameRule.KEEP_INVENTORY, true);
                    Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "execute in minecraft:" + netherName + " run gamerule locator_bar false");
                    survivalNetherWorld.setGameRule(GameRule.SEND_COMMAND_FEEDBACK, true);
                });
                plugin.getLogger().info("Nether world created: " + netherName);
            }

            // Create the linked end world
            String endName = survivalWorldName + "_the_end";
            survivalEndWorld = new WorldCreator(endName)
                    .environment(World.Environment.THE_END)
                    .seed(survivalWorld.getSeed())
            ...
            if (survivalEndWorld != null) {
                survivalEndWorld.setGameRule(GameRule.KEEP_INVENTORY, true);
                survivalEndWorld.setDifficulty(org.bukkit.Difficulty.NORMAL);
                Bukkit.getScheduler().runTask(plugin, () -> {
                    survivalEndWorld.setGameRule(GameRule.SEND_COMMAND_FEEDBACK, false);
                    survivalEndWorld.setGameRule(GameRule.ANNOUNCE_ADVANCEMENTS, false);
                    survivalEndWorld.setGameRule(GameRule.KEEP_INVENTORY, true);
                    Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "execute in minecraft:" + endName + " run gamerule locator_bar false");
                    survivalEndWorld.setGameRule(GameRule.SEND_COMMAND_FEEDBACK, true);
                });
                plugin.getLogger().info("End world created: " + endName);
            }
        }
    }
    ...
    /**
     * Get the survival nether world (linked to survival world).
     */
    public World getSurvivalNetherWorld() { return survivalNetherWorld; }

    /**
     * Get the survival end world (linked to survival world).
     */
    public World getSurvivalEndWorld() { return survivalEndWorld; }

    /**
     * Check if the given world is part of the survival dimension group
     * (overworld, nether, or end).
     */
    public boolean isSurvivalDimension(World world) {
        if (world == null) return false;
        return world.equals(survivalWorld) || world.equals(survivalNetherWorld) || world.equals(survivalEndWorld);
    }
    ...
    public void cleanup() {
        if (survivalNetherWorld != null) {
            unloadAndDeleteWorld(survivalNetherWorld);
            survivalNetherWorld = null;
        }
        if (survivalEndWorld != null) {
            unloadAndDeleteWorld(survivalEndWorld);
            survivalEndWorld = null;
        }
        if (survivalWorld != null) {
            unloadAndDeleteWorld(survivalWorld);
            survivalWorld = null;
