...
public class GameCommands implements CommandExecutor, org.bukkit.command.TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (label.equalsIgnoreCase("start")) {
            if (!gm.isStarting()) {
                if (gm.isWaiting()) {
                    sender.sendMessage("§cGunakan /nextround untuk melanjutkan permainan!");
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        if (command.getName().equalsIgnoreCase("settimer") || alias.equalsIgnoreCase("settimer")) {
            if (args.length == 1) {
                return List.of("5", "7", "10", "15");
            }
            return List.of();
        }
        return null; // Return null to fallback to online players if needed for other commands
    }
    ...
    private void startHidePhase() {
        ...
        SilentCommands.run("difficulty normal");
